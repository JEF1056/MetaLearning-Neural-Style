...
static vector<boost::shared_ptr<Net > > nets_;
...
void entry_save_model(PyObject* weights_raw, string layer_name_file, string base_model_name, string predict_model_name) 
{
  ...
  char layer_name[100];
 	...
 	if (layer_name_list.size() != PyList_Size(weights_raw))
 	{
 		//std::cout<<layer_name_list.size()<<", "<<PyList_Size(weights_raw)<<'\n';
 		throw std::runtime_error("layer name and blob size not match");
	}
  for (int i = 0; i < PyList_Size(weights_raw); ++i)
  {
...
PyObject* entry_get_weights(int hnet) 
{
	...
	boost::shared_ptr<Net > net_ = nets_[hnet];

	
	
  const vector<boost::shared_ptr<Layer > >& layers = net_->layers();
  ...
  {
    vector<boost::shared_ptr<Blob > >& layer_blobs = layers[i]->blobs();
  ...
  for (unsigned int i = 0; i < layers.size(); ++i) 
  {
    ...
    for (unsigned int j = 0; j < layer_blobs.size(); ++j) 
    {
      ...
      float* weights_ptr = reinterpret_cast<float*>(PyArray_DATA(blobs_list->ob_item[j]));		
		...
		py_layers_name_->ob_item[py_layer_index] = PyUnicode_FromString(layer_names[i].c_str());
...
void entry_init(char*model_file,char * weight_file) 
{
  ...
  vector<boost::shared_ptr<Blob > > net_input_blobs;
  ...
  for (int i = 0; i < net_param.input_blob_size(); ++i)
  {
    ...
    boost::shared_ptr<Blob > blob_pointer(new Blob(num, channels, height, width));
...
PyObject* entry_forward(int hNet, PyObject* bottom_raw) 
{
  ...
  const vector<boost::shared_ptr<Blob > >& input_blobs = net_->input_blobs();
  if (PyList_Size(bottom_raw) != input_blobs.size()) 
  ...
  net_->Forward();
 	const vector<boost::shared_ptr<Blob > >& output_blobs = net_->output_blobs();
